...
void Panel::Panel::Draw(hotline::ActionSet &set) {
  ...
  auto io = ImGui::GetIO();
  ImVec2 position{io.DisplaySize.x * config.windowPos.x,
                  io.DisplaySize.y * config.windowPos.y};
  ImVec2 size{io.DisplaySize.x * config.windowSize.x,
              io.DisplaySize.y * config.windowSize.y};
  ...
  ImGui::BeginChild("header",
                    {windowSize.x * config.headerPanelSize.x,
                     windowSize.y * config.headerPanelSize.y},
                    true, config.windowFlags);
  ImGui::Text(
      "Panel - lightweight framework for action executing with xml layout");
  ImGui::EndChild();
  ImGui::SameLine();
  ...
  if (ImGui::Button("Close", {ImGui::GetContentRegionAvail().x,
                              windowSize.y * config.headerPanelSize.y})) {
    _onExitCallback();
  }
  ...
  ImGui::SetWindowFontScale(contextConfig.windowHotkeyScale *
                            contextConfig.scaleFactor);
  ImGui::SetCursorPos(cursorPointBeforeButton);
  ImGui::TextColored(config.hotkeyColor, ("[esc]"));
  ImGui::SetWindowFontScale(contextConfig.windowFontScale *
                            contextConfig.scaleFactor);
...
void Panel::Panel::Reset() { _rootElement->Reset(); }
...
void Panel::Panel::InitFromXml() {
  ...
  auto root = doc.root();
  _rootElement =
      std::move(ContextFactory::InitContext(doc.root().first_child()));
}
